...
// // const sqlQuery = `SELECT * FROM your_table_name WHERE key_column = ? AND value_column = ?`;
// //fetching location data from mysql database
// app.get('/all/:Location', (req,res) => {
//     connection.query(sql_query, (err, result) => {
//         console.log(req.params.Location);
//         const storedFetchedData = result.filter((item) => item.Location.toString() === req.params.Location)
//         return res.json(storedFetchedData);

//     })
// })

// // fetching mskus data from mysql database
// app.get('/all/:MSKU', (req,res) => {
//     connection.query(sql_query, (err,result) => {
//         if (err) console.log('msku error while fetching data fom db')
//         const storeMskusData = result.filter((item) => item.MSKU.toString() === req.params.MSKU)
//         return res.json(storeMskusData);
//     })
// })

// fetching single msku data from location filtered data
// app.get('/all/:Location/:MSKU', (req,res) => {
//     connection.query(sql_query, (err,result) => {
//         if (err) console.log('msku error while fetching data fom location filtered data ');
//         const storeFetchedData = result.filter((item) => item.Location.toString() === req.params.Location);
//         const storeMskuData = storeFetchedData.filter((item) => item.MSKU.toString() === req.params.MSKU);
//         return res.json(storeMskuData);
//     })
// })
...
//     }
//     return console.log('mysql connection is still on')
// })



//  note
// ID starts from 3044-9119
// filter for Location, Quantity, ID, MSKU, Item_name, 